...
PG_FUNCTION_INFO_V1(pg_onnx_internal_describe_session);
...
Datum pg_onnx_internal_describe_session(PG_FUNCTION_ARGS) {
	// check args count and is null
	if (PG_NARGS() < 2 || PG_ARGISNULL(0) || PG_ARGISNULL(1))
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid argument")));
	...
	try {
		return task_describe_session(state, fcinfo, name, version);
	} catch (std::exception &e) {
		elog(ERROR, "%s: %s", __FUNCTION__, e.what());
	}
}
